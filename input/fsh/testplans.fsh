...
//
// Validates that the Trust Anchor correctly receives, validates, stores, and responds to DID Document
// submissions from VHL Sharers and VHL Receivers.  Covers structural validation, cryptographic
// material checks, identity authentication, cataloguing, revocation support, and all defined HTTP
...
// single-DID GET, bulk GET, mCSD-based queries, active-document-only filtering, authentication and
// authorisation checks (401/403), 404 handling, optional response signing, and receiver post-
...
//
// Validates that the VHL Sharer correctly constructs and submits a DID Document to the Trust Anchor.
// Covers mandatory DID Document elements (@context, id, verificationMethod), verification method
// content (id, type, controller, publicKeyJwk/publicKeyMultibase), private-key exclusion,
...
// payload construction (url, key, exp, flag, label, v), mandatory manifest URL parameters, HCERT/CWT
// encoding pipeline (COSE signing → ZLIB compression → Base45 encoding → HC1: prefix), QR code
// generation (ISO/IEC 18004:2015, error-correction Q, Alphanumeric mode), and error OperationOutcome
// responses.
* testCase[=].testRun[+].narrative = """
Execute all scenarios in the Gherkin feature file
